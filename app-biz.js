...
function renderSettings(root){
  ...
  root.innerHTML=`
    ...
    <div class="page-block" id="zySyncBlock">${blockHead('云端同步（跨设备实时同步，免费，不用腾讯云）',`<button class="primary" onclick="zyTestConn()">测试连接</button><button class="ghost" onclick="zyPullNow()">立即下载云端</button><button class="ghost" onclick="zyPushNow()">立即上传本地</button>`)}<div class="block-body"><div class="tip-line" style="margin-bottom:10px;">本平台已部署在 <b>GitHub Pages 永久地址</b>。开启云端同步后，<b>手机/电脑/平板的数据实时一致</b>（手机端注册秒级出现在电脑审核中心），数据以<b>加密形式</b>存进你的 Supabase 云端（免费），即使云端数据被公开读取也无法解读。仅管理角色可配置与读取。</div><div class="form-grid cols-2"><label>Supabase 项目地址<input id="zyUrl" placeholder="https://xxxx.supabase.co" value="${esc((window.ZY&&ZY.loadCfg()||{}).url||'')}"></label><label>Supabase 匿名密钥（Publishable/Anon Key）<input id="zyKey" placeholder="sb_publishable_..." value="${esc((window.ZY&&ZY.loadCfg()||{}).key||'')}"></label><label>云同步邮箱<input id="zyEmail" placeholder="任意邮箱（用于云端鉴权）" value="${esc((window.ZY&&ZY.loadCfg()||{}).email||'')}"></label><label>云同步密码（重要：也是数据加密密钥，务必记牢）<input id="zyPwd" type="password" placeholder="至少 8 位" value="${esc((window.ZY&&ZY.loadCfg()||{}).pwd||'')}"></label></div><div class="mt-12" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;"><button class="primary" style="height:38px;padding:0 24px;" onclick="zySaveCfg()">保存并连接云端</button><span class="f12 c-3" id="zyStatus">未连接</span></div><div class="tip-line mt-12" style="font-size:12px;color:var(--ink-3);">首次使用：① 先在 Supabase 控制台 SQL Editor 执行 <b>supabase-setup.sql</b>（仓库根目录有该文件，建表一次即可）→ ② 回来填上面配置点「保存并连接云端」→ ③ 若提示需邮箱确认，去邮箱点确认后重连。所有设备填相同配置即互通。</div></div></div>
