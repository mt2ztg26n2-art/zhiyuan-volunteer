...
window.auditApprove=(id)=>{const u=DB.users.find(x=>x.id===id);if(u){const before={pending:!!u.pending,activated:!!u.activated,status:u.status};/* CloudBase 模式：审核走云函数（服务端角色+部门校验+通知本人） */if(window.ZY&&ZY.cloud&&ZY.auditApprove){ZY.auditApprove(id).then(r=>{if(r.ok){toast(r.msg||'已审核通过','ok');ZY.pull().then(()=>{if(window.renderAuditPending)renderAuditPending();if(window.updateNotifyBadge)updateNotifyBadge();});}else toast(r.msg||'审核失败','err');});return;}u.activated=true;u.pending=false;u.status=u.status||'正常在岗';u.updatedAt=Date.now();saveDB();pushNotify({to:u.name,kind:'sys',title:'注册审核通过',content:`${u.name}，您的志愿者注册已通过审核，现在可以使用账号登录。`});pushLog('审核','通过 '+u.name+' 的注册');pushTrace('审核通过','注册: '+u.name+' ('+u.idCard+')',before,{pending:false,activated:true,status:u.status});/* 云端：写入审核状态 + 删除已处理注册条目 */if(window.ZYStatus)ZYStatus.set(u.idCard,'approved');if(window.ZYReg&&u._cloudRegId)ZYReg.remove(u._cloudRegId);if(window.ZY)ZY.push();renderAuditPending();toast('已审核通过','ok')}};
window.auditRejectUser=(id)=>{const u=DB.users.find(x=>x.id===id);if(!u)return;confirmDialog(`确认驳回 <b>${esc(u.name)}</b> 的注册申请？`,()=>{const before={pending:!!u.pending,activated:!!u.activated};/* CloudBase 模式：驳回走云函数（服务端校验+移除+通知本人+释放身份证） */if(window.ZY&&ZY.cloud&&ZY.auditReject){ZY.auditReject(id).then(r=>{if(r.ok){toast(r.msg||'已驳回','ok');ZY.pull().then(()=>{if(window.renderAuditPending)renderAuditPending();if(window.updateNotifyBadge)updateNotifyBadge();});}else toast(r.msg||'驳回失败','err');});return;}u.pending=false;u.activated=false;u.updatedAt=Date.now();saveDB();pushNotify({to:u.name,kind:'sys',title:'注册审核驳回',content:`${u.name}，您的注册申请未通过审核，请联系团委管理员。`});pushLog('审核','驳回 '+u.name+' 的注册');pushTrace('审核驳回','注册: '+u.name+' ('+u.idCard+')',before,{pending:false,activated:false});if(window.ZYStatus)ZYStatus.set(u.idCard,'rejected');if(window.ZYReg&&u._cloudRegId)ZYReg.remove(u._cloudRegId);if(window.ZY)ZY.push();renderAuditPending();toast('已驳回','ok')},'驳回注册')};
window.auditQuery=()=>{
