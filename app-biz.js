...
function renderSettings(root){
  ...
  root.innerHTML=`
    ...
    <div class="page-block">${blockHead('专业部 / 班级 管理（自定义）','<button class="primary" style="height:30px;padding:0 14px;" onclick="saveDeptMgr()">保存</button><button onclick="addDept()">+ 新增专业部</button>')}<div class="block-body" id="deptMgr"></div></div>
...
window.delClass=(dept,name)=>confirmDialog(`确认删除班级「${name}」？（专业部「${dept}」仍会保留）`,()=>{DB.dictionaries.classes[dept]=(DB.dictionaries.classes[dept]||[]).filter(c=>c!==name);saveDB();if(window.ZY)ZY.push();renderDeptMgr();toast('已删除班级，专业部已保留','ok')});
window.saveDeptMgr=()=>{saveDB();if(window.ZY)ZY.push();renderDeptMgr();toast('专业部/班级设置已保存并同步到云端','ok')};
